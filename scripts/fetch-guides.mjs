...
async function fetchGuideSection(title){
  var secRes = await fetch(WIKI_API + "?action=parse&page=" + encodeURIComponent(title) + "&format=json&prop=sections");
  ...
  var htmlRes = await fetch(WIKI_API + "?action=parse&page=" + encodeURIComponent(title) + "&format=json&prop=text&section=" + sec.index);
